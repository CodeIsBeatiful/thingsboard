...
import {concatAll, concatMap, map, mergeMap, switchAll, switchMap, take} from "rxjs/operators";
...
export class WhitelabelUtilsService {
  ...
  public addLoginTheme() {
    if(!this.loginThemeCssElm){
      let loginThemeCssElm = this._document.getElementById("tb-login-theme");
      if(!loginThemeCssElm){
        loginThemeCssElm = this._document.createElement('style');
  ...
  public addAppTheme(){
    if(!this.appThemeCssElm){
      let appThemeCssElm = this._document.getElementById("tb-app-theme");
      if(!appThemeCssElm){
        appThemeCssElm = this._document.createElement('style');
  ...
  private process(isSetup?: boolean) {
    ...
    //set icon
    this._document.querySelector(("link[rel*='icon']")).setAttribute('href', this.whiteLabeling.faviconUrl);
  }
}
